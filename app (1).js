...
function renderGate(errorMsg){
  var app = document.getElementById('app');
  if(!app) return;
  app.innerHTML = '<div class="boot-screen"><form class="boot-card" data-form="gate-password">'
   + '<img class="brand-mark" src="/logo.jpg" alt="Amanda Pereira - Arts in Nails" style="margin:0 auto 14px;width:72px;height:72px;border-radius:18px">'
   + '<h1 style="text-align:center;font-size:20px;margin-bottom:6px">Painel da Amanda</h1>'
   + '<p class="page-sub" style="text-align:center;margin-bottom:18px">Digite a senha para acessar.</p>'
   + (errorMsg ? '<p style="color:var(--critical);font-size:13px;text-align:center;margin-bottom:10px">'+escapeHtml(errorMsg)+'</p>' : '')
   + '<div class="field"><label>Senha</label><input type="password" name="password" autofocus></div>'
